...
def handle_agent_query():
    ...
    # --- NEW: Endpoint for Teacher Grading and Comments ---
    @app.route('/api/submissions/grade', methods=['POST'])
    def grade_submission():
        """
        Allows teachers/admins to add or update a grade and comment for a submission.
        Expects: submissionId, grade, comment, auth token
        """
        try:
            data = request.get_json()
            ...
            submissions_ref = firestore_db.collection('artifacts').document(app_id).collection('submissions')
            doc_ref = submissions_ref.document(submission_id)
            update_data = {}
            ...
            if not update_data:
                return jsonify({"error": "No grade or comment provided."}), 400
            doc_ref.update(update_data)
            return jsonify({"success": True, "submissionId": submission_id, "updated": update_data}), 200
        except Exception as e:
            print(f"Grade/comment error: {e}")
            return jsonify({"error": str(e)}), 500

    # --- NEW: Endpoint to Retrieve Grade/Comment for a Submission ---
    @app.route('/api/submissions/feedback', methods=['GET'])
    def get_submission_feedback():
        """
        Retrieve grade and comment for a submission. Expects: submissionId, auth token
        """
        try:
            ...
            data = doc.to_dict()
            feedback = {"grade": data.get('grade'), "comment": data.get('comment')}
            return jsonify({"submissionId": submission_id, "feedback": feedback}), 200
        except Exception as e:
...
def delete_expired_solved_problems():
    ...
    # --- NEW: Endpoint to Retrieve Submissions (with pagination & role check) ---
    @app.route('/api/submissions/list', methods=['GET'])
    def list_submissions():
        """
        Retrieve submissions for review. Supports pagination and role-based access.
        Query params: assignmentId, studentId, pageSize, pageToken
        """
        try:
            auth_token = request.headers.get('Authorization')
            if not auth_token:
            ...
            # pageToken is not used for Firestore streaming, but could be used for custom pagination

            app_id = os.getenv("FIREBASE_APP_ID", "default-app-id")
            submissions_ref = firestore_db.collection('artifacts').document(app_id).collection('submissions')
            query = submissions_ref
            if assignment_id:
                query = query.where('assignmentId', '==', assignment_id)
            if student_id:
                query = query.where('studentId', '==', student_id)

            # Only teachers/admins can view all submissions; students can only view their own
            if user_role not in ['Teacher', 'Admin'] and student_id and student_id != user_id:
                return jsonify({"error": "Permission denied."}), 403
            ...
            for doc in docs:
                sub = doc.to_dict()
                sub['submissionId'] = doc.id
                submissions.append(sub)

            # Pagination: Firestore does not support offset, so use pageToken (last doc ID)
            next_page_token = None
            if len(submissions) == page_size:
                next_page_token = submissions[-1]['submissionId']
