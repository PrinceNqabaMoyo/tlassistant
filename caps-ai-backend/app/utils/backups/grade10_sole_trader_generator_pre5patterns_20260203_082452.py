...
def _make_transaction_analysis_question(
    *,
    ...
    def _make_owner_withdrawal(*, amt: float, nr: str) -> List[Dict[str, Any]]:
        ...
        row1 = _tx(nr=nr, source="Cheque counterfoil", journal="CPJ", dr="Drawings", cr="Bank", amount=float(amt), a=-amt, o=-amt, l=0.0)
        row1["tag"] = "owner_withdrawal"
        return [row1]

    def _make_fixed_deposit_investment(*, amt: float, nr: str) -> List[Dict[str, Any]]:
        # Invest in fixed deposit - asset swap from Bank to Fixed deposit
        row1 = _tx(nr=nr, source="Bank statement", journal="", dr="Fixed deposit", cr="Bank", amount=float(amt), a=0.0, o=0.0, l=0.0)
        row1["tag"] = "fixed_deposit_investment"
        return [row1]

    def _make_petty_cash_imprest_restoration(*, amt: float, nr: str) -> List[Dict[str, Any]]:
        # Restore petty cash imprest from bank
        row1 = _tx(nr=nr, source="Cheque counterfoil", journal="CPJ", dr="Petty cash", cr="Bank", amount=float(amt), a=0.0, o=0.0, l=0.0)
    ...
    def _make_owner_taking_stock(*, cost: float, nr: str) -> List[Dict[str, Any]]:
        ...
        row1 = _tx(nr=nr, source="Journal voucher", journal="GJ", dr="Drawings", cr="Trading stock", amount=float(cost), a=-cost, o=-cost, l=0.0)
    ...
    def _make_interest_on_current_account(*, amt: float, nr: str) -> List[Dict[str, Any]]:
        ...
        row1 = _tx(nr=nr, source="Bank statement", journal="", dr="Bank", cr="Interest income", amount=float(amt), a=+amt, o=+amt, l=0.0)
    ...
    def _make_debtor_settlement_discount_unfavourable_bank(*, owed: float, disc_pct: float, nr: str) -> List[Dict[str, Any]]:
        # Bank account is unfavourable (overdraft). A receipt reduces the overdraft (liability decreases).
        disc = float(_round_money(owed * (disc_pct / 100.0)))
        bank_amt = float(_round_money(owed - disc))
        row1 = _tx(nr=nr, source="Duplicate receipt", journal="", dr="Bank", cr="Debtors control", amount=bank_amt, a=0.0, o=0.0, l=-bank_amt)
        row1["tag"] = "bank_unfavourable_receipt"
        row2 = _tx(nr="", source="", journal="", dr="Discount allowed", cr="Debtors control", amount=disc, a=0.0, o=-disc, l=0.0)
    ...
    def _make_overdraft_interest(*, amt: float, nr: str) -> List[Dict[str, Any]]:
        ...
        row1 = _tx(nr=nr, source="Bank statement", journal="", dr="Interest expense", cr="Bank", amount=float(amt), a=0.0, o=-float(amt), l=+float(amt))
    ...
    def _build_tx_pool() -> List[Tuple[str, List[Dict[str, Any]]]]:
        ...
        tx_counter += 1
        nr = str(tx_counter)
        out.append(("owner_withdrawal", _make_owner_withdrawal(amt=float(_pick_amount([500, 1000, 1500, 2000])), nr=nr)))
        tx_counter += 1
        nr = str(tx_counter)
        out.append(("fixed_deposit_investment", _make_fixed_deposit_investment(amt=float(_pick_amount([5000, 10000, 15000])), nr=nr)))
        tx_counter += 1
        nr = str(tx_counter)
        out.append(("petty_cash_imprest", _make_petty_cash_imprest_restoration(amt=float(_pick_amount([500, 800, 1200])), nr=nr)))
        tx_counter += 1
        nr = str(tx_counter)
        out.append(("owner_taking_stock", _make_owner_taking_stock(cost=float(_pick_amount([300, 500, 750, 1000])), nr=nr)))
        tx_counter += 1
        nr = str(tx_counter)
        out.append(("interest_current_account", _make_interest_on_current_account(amt=float(_pick_amount([50, 100, 150, 200])), nr=nr)))
        tx_counter += 1
        return out
    ...
    def _hint_for_row(*, t: Dict[str, Any], current_tx: str) -> str:
        ...
        if nr0.strip():
            ...
            if tag0 == "owner_withdrawal":
                return "Owner withdrawal: Drawings increases (debit), Bank decreases (credit). Decreases Assets and Owner's Equity."
