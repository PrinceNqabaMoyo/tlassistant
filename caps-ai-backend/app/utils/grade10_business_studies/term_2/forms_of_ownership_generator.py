...
def _generate_concepts_mcq(count: int, difficulty: str) -> list:
    """Generate MCQ concept questions for Forms of ownership."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner in South Africa. "
        "Generate a JSON list of MCQ questions on 'Topic 10: Forms of ownership'. "
        "Focus on defining the different forms (Sole Trader, Partnership, Close Corporation, Private Company, Public Company, Personal Liability Company, State-Owned Company, Non-Profit Company) "
        "and identifying their characteristics such as capacity, taxation, liability (limited vs unlimited), and continuity."
...
def _generate_discussion(count: int, difficulty: str) -> list:
    """Generate typed discussion questions for Forms of ownership."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner. "
        "Generate a JSON list of semantic/essay/discussion questions on 'Topic 10: Forms of ownership'. "
        "Focus on: evaluating a given scenario to recommend the best form of ownership, "
        "discussing the advantages and disadvantages of specific forms of ownership (e.g., Sole Trader vs Partnership, Private vs Public Company), "
        "and explaining concepts like limited liability, continuity, and double taxation."
    )
...
def generate_forms_of_ownership(subskill: str, difficulty: str, count: int) -> dict:
    """
    Main router for Forms of ownership generation.
    """
    try:
        questions = []
