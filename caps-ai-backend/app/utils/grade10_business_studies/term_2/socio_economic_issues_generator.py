...
def _generate_concepts_mcq(count: int, difficulty: str) -> list:
    """Generate MCQ concept questions for Socio-economic issues."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner in South Africa. "
        "Generate a JSON list of MCQ questions on 'Topic 7: Socio-economic issues'. "
        "Focus on defining issues like inequality, poverty, inclusivity, unemployment, HIV/AIDS, piracy, counterfeiting, bootlegging, strikes, and crime. "
...
def _generate_discussion(count: int, difficulty: str) -> list:
    """Generate typed discussion questions for Socio-economic issues."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner. "
        "Generate a JSON list of semantic/essay/discussion questions on 'Topic 7: Socio-economic issues'. "
        "Focus on: explaining the negative impact of specific issues (e.g. HIV/AIDS, strikes, crime, piracy) on businesses, "
        "and suggesting ways in which businesses can address or overcome these challenges."
    )
...
def generate_socio_economic_issues(subskill: str, difficulty: str, count: int) -> dict:
    """
    Main router for Socio-economic issues generation.
    """
    try:
        questions = []
