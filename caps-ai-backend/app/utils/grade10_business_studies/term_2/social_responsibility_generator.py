...
def _generate_concepts_mcq(count: int, difficulty: str) -> list:
    """Generate MCQ concept questions for Social Responsibility."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner in South Africa. "
        "Generate a JSON list of MCQ questions on 'Topic 8: Social responsibility'. "
        "Focus on defining Corporate Social Responsibility (CSR), Corporate Social Investment (CSI), "
        "and the difference between the two. Also cover why businesses should be socially responsible."
...
def _generate_discussion(count: int, difficulty: str) -> list:
    """Generate typed discussion questions for Social Responsibility."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner. "
        "Generate a JSON list of semantic/essay/discussion questions on 'Topic 8: Social responsibility'. "
        "Focus on: evaluating CSR/CSI initiatives, discussing the impact of CSI on communities and the business itself (advantages/disadvantages), "
        "and suggesting CSR programs a business could implement for its employees or the local community."
    )
...
def generate_social_responsibility(subskill: str, difficulty: str, count: int) -> dict:
    """
    Main router for Social Responsibility generation.
    """
    try:
        questions = []
