...
def _generate_concepts_mcq(count: int, difficulty: str) -> list:
    """Generate MCQ concept questions for Entrepreneurial qualities."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner in South Africa. "
        "Generate a JSON list of MCQ questions on 'Topic 9: Entrepreneurial qualities'. "
        "Focus on defining what an entrepreneur is, and identifying key qualities such as risk-taking, "
        "innovation, perseverance, leadership, and adaptability. "
...
def _generate_discussion(count: int, difficulty: str) -> list:
    """Generate typed discussion questions for Entrepreneurial qualities."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner. "
        "Generate a JSON list of semantic/essay/discussion questions on 'Topic 9: Entrepreneurial qualities'. "
        "Focus on: evaluating a scenario to identify which entrepreneurial qualities a person is displaying, "
        "explaining why certain qualities (like risk-taking or perseverance) are vital for business success, "
        "and discussing the difference between an entrepreneur and an intrapreneur."
    )
...
def generate_entrepreneurial_qualities(subskill: str, difficulty: str, count: int) -> dict:
    """
    Main router for Entrepreneurial qualities generation.
    """
    try:
        questions = []
