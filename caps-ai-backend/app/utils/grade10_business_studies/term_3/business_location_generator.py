...
def _generate_concepts_mcq(count: int, difficulty: str) -> list:
    """Generate MCQ concept questions for Topic 14: Business Location Decisions."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner in South Africa following the CAPS curriculum. "
        "Generate a JSON list of MCQ questions on 'Topic 14: Business Location Decisions'. "
        "Focus on the importance of business location and the factors impacting where a business is located (proximity to market, cost and availability of labour, access to raw materials and suppliers, transport and infrastructure, cost of premises, tax rebates, safety, competition and legislation). "
...
def _generate_discussion(count: int, difficulty: str) -> list:
    """Generate typed discussion/essay questions for Topic 14: Business Location Decisions."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner following the CAPS curriculum. "
        "Generate a JSON list of semantic/essay/discussion questions on 'Topic 14: Business Location Decisions'. "
        "Focus on: explaining why location is an important business decision, and evaluating the factors a business must consider when choosing a location for a given scenario."
    )
...
def generate_business_location(subskill: str, difficulty: str, count: int) -> dict:
    """Main router for Topic 14: Business Location Decisions generation."""
    try:
        questions = []
