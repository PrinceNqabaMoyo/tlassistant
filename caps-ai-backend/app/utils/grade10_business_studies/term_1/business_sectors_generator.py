...
def _generate_concepts_mcq(count: int, difficulty: str) -> list:
    """Generate MCQ concept questions for Business Sectors."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner in South Africa. "
        "Generate a JSON list of MCQ questions on 'Topic 6: Business Sectors'. "
        "Focus on defining and distinguishing between the Primary, Secondary, and Tertiary sectors. "
...
def _generate_discussion(count: int, difficulty: str) -> list:
    """Generate typed discussion questions for Business Sectors."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner. "
        "Generate a JSON list of semantic/essay/discussion questions on 'Topic 6: Business Sectors'. "
        "Focus on: explaining the link between the three sectors (e.g. how primary provides raw materials to secondary, which sells via tertiary), "
        "classifying businesses into sectors from a scenario, and discussing the importance of each sector to the economy."
    )
...
def generate_business_sectors(subskill: str, difficulty: str, count: int) -> dict:
    """
    Main router for Business Sectors generation.
    """
    try:
        questions = []
