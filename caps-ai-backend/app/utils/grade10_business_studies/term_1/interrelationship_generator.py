...
def _generate_concepts_mcq(count: int, difficulty: str) -> list:
    """Generate MCQ concept questions for Interrelationship."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner in South Africa. "
        "Generate a JSON list of MCQ questions on 'Topic 5: The Interrelationship of the Micro, Market and Macro Environments'. "
        "Focus on how the three environments affect each other, especially how micro adapts to market/macro, and how business influences the market. "
...
def _generate_discussion(count: int, difficulty: str) -> list:
    """Generate typed discussion questions for Interrelationship."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner. "
        "Generate a JSON list of semantic/essay/discussion questions on 'Topic 5: The Interrelationship of Environments'. "
        "Focus on: explaining the relationship between the micro, market and macro environments, "
        "discussing how a change in the macro environment (e.g. new law) affects the micro environment, "
        "and how a business can influence its market environment (e.g. suppliers, competitors)."
    )
...
def generate_interrelationship(subskill: str, difficulty: str, count: int) -> dict:
    """
    Main router for Interrelationship generation.
    """
    try:
        questions = []
