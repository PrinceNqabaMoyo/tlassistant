...
def _generate_concepts_mcq(count: int, difficulty: str) -> list:
    """Generate MCQ concept questions for Market Environment."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner in South Africa. "
        "Generate a JSON list of MCQ questions on 'Topic 3: The Market Environment'. "
        "Focus on the components of the market environment (consumers, suppliers, intermediaries, competitors, civil society, NGOs, CBOs, regulators, strategic allies). "
...
def _generate_discussion(count: int, difficulty: str) -> list:
    """Generate typed discussion questions for Market Environment."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner. "
        "Generate a JSON list of semantic/essay/discussion questions on 'Topic 3: The Market Environment'. "
        "Focus on: explaining reasons why NGOs and CBOs form part of the market environment, discussing the role of suppliers/competitors/intermediaries, "
        "identifying market environment components from scenarios, and explaining the meaning of the market environment."
    )
...
def generate_market_environment(subskill: str, difficulty: str, count: int) -> dict:
    """
    Main router for Market Environment generation.
    """
    try:
        questions = []
