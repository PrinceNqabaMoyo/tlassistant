...
def _generate_concepts_mcq(count: int, difficulty: str) -> list:
    """Generate MCQ concept questions for Business Functions."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner in South Africa. "
        "Generate a JSON list of MCQ questions on 'Topic 2: Business Functions'. "
        "Focus on the differences between management and leadership, the levels of management (Top, Middle, Lower), "
        "the management tasks (Planning, Organising, Leading, Controlling, Risk management), "
        "organisational structures (Functional, Project, Matrix), and the administration function (information management). "
...
def _generate_discussion(count: int, difficulty: str) -> list:
    """Generate typed discussion questions for Business Functions."""
    system_prompt = (
        "You are an expert Grade 10 Business Studies examiner. "
        "Generate a JSON list of semantic/essay/discussion questions on 'Topic 2: Business Functions'. "
        "Focus on topics like: differences between leadership and management, explaining management tasks (e.g., planning, organising), "
        "responsibilities of different levels of management, discussing types of organisational structures (functional, project, matrix), "
        "or activities of the administration function. "
        "These are long-form typed questions."
    )
...
def generate_business_functions(subskill: str, difficulty: str, count: int) -> dict:
    """
    Main router for Business Functions generation.
    """
    try:
        questions = []
