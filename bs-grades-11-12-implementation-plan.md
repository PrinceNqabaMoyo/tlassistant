...
- **Data source:** `caps-ai-backend/curriculum_docs/BusinessStudies_Gr{10,11,12}/Term-*/` — each topic is a standalone `.md` file (e.g., `2 Business functions and the activities of the business.md`).
- Create `parse_md_sections(md_path)` that reads one of these `.md` topic files and returns a list of sections derived from the file's `###` / `####` headings (e.g., `Understanding business functions`, `The eight business functions`, `Management tasks`).
...
- **Primary source for progression:** `caps-ai-backend/curriculum_docs/BusinessStudies_Gr{10,11,12}/Term-*/` contains the `.md` topic files that the section parser reads to build scaffold steps. These are the curriculum-aligned files that drive progression.
- **Supplementary source:** `caps-wiki/business-studies/grade-11/` and `grade-12/` contain additional notes and reference material for the agent and content generation.
- Ensure every topic has a corresponding `.md` in `curriculum_docs/`.
- Follow the same heading structure (`### Section Name`, `#### Sub-section`) so the section parser works consistently across grades.
