...
export default function App() {
  ...
  const topLevelPage = showLandingPage ? 'landing' : (effectiveCurrentUser ? 'app' : 'auth');
  const shouldRenderStandaloneLandingPage = showLandingPage && (!effectiveCurrentUser || hasResolvedInitialAuthViewRef.current);
  ...
  }, [authLoading, showSplash, effectiveCurrentUser, setShowLandingPage]);
  ...
  useEffect(() => {
    ...
    if (isCAPSContext && effectiveRole === 'teacher') {
      document.title = 'Fundile | CAPS Teaching assistant';
      return;
    }

    if (isCAPSContext && effectiveRole === 'student') {
      document.title = 'Fundile | CAPS Learning assistant';
      return;
    }
  ...
  useEffect(() => {
    ...
    const handlePopState = (event) => {
      const requestedPage = event.state?.fundilePage;
  ...
  useEffect(() => {
    ...
    if (!hasInitializedBrowserHistoryRef.current) {
      if (topLevelPage === 'app') {
        window.history.replaceState({ fundilePage: 'landing' }, '', window.location.href);
        window.history.pushState({ fundilePage: 'app' }, '', window.location.href);
      } else {
        window.history.replaceState({ fundilePage: topLevelPage }, '', window.location.href);
      }
    ...
    if (previousTopLevelPage === topLevelPage) {
      return;
    }

    if (topLevelPage === 'auth' && previousTopLevelPage === 'landing') {
      window.history.pushState({ fundilePage: 'auth' }, '', window.location.href);
    } else if (topLevelPage === 'app' && previousTopLevelPage === 'auth') {
      window.history.replaceState({ fundilePage: 'app' }, '', window.location.href);
    } else if (topLevelPage === 'app' && previousTopLevelPage === 'landing') {
      window.history.pushState({ fundilePage: 'app' }, '', window.location.href);
    } else if (topLevelPage === 'auth') {
      window.history.replaceState({ fundilePage: 'auth' }, '', window.location.href);
    }
  ...
  const getAgentResponse = useCallback(async (input, history) => {
    ...
    const apiUrl = 'http://127.0.0.1:5001/api/agent'; // Ensure this is your actual API endpoint
  ...
  const handleSubmit = useCallback(async (questionId, questionText, expectedAnswerType, studentAnswer, questionSolution, currentQuestionBlockRef) => {
    ...
    try {
      ...
      const evalResponse = await fetch('http://127.0.0.1:5001/api/evaluate-typed', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  if (showSplash) {
    return <SplashScreen onComplete={() => {
      if (typeof window !== 'undefined') {
        window.sessionStorage.setItem('fundileSeenSplash', 'true');
      }
      setShowSplash(false);
      setShowLandingPage(true);
    }} />;
  ...
  if (shouldRenderStandaloneLandingPage) {
    return <LandingPage onGetStarted={() => setShowLandingPage(false)} palette={brandPalette} />;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      <BackButton
        ...
        currentView={view}
      />
      <Header currentUser={effectiveCurrentUser} onLogout={handleLogout} pendingAssignments={pendingAssignments} onStartAssignment={handleStartAssignment} setView={setView} superAdminMode={superAdminMode} setSuperAdminMode={setSuperAdminMode} brandPalette={brandPalette} setBrandPalette={setBrandPalette} />
