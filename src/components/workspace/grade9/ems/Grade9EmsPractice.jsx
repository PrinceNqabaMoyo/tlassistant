...
const Grade9EmsPractice = ({
    onBack,
    g9EmsVisualAidsOpen,
    setg9EmsVisualAidsOpen,
    g9EmsPracticeDifficulty,
    setg9EmsPracticeDifficulty,
    g9EmsPracticeSubskill,
    setg9EmsPracticeSubskill,
    g9EmsPracticeSeed,
    setg9EmsPracticeSeed,
    subskills,
    fetchGrade9EmsPractice,
    g9EmsPracticeLoading,
    g9EmsPracticeError,
    g9EmsPracticeQuestions,
    g9EmsPracticeAnswers,
    setg9EmsPracticeAnswers,
    g9EmsPracticeFeedback,
    setg9EmsPracticeFeedback,
    renderGrade9EmsVisualAids,
    hideConfig,
    isSuperAdmin,
}) => {
    const questions = Array.isArray(g9EmsPracticeQuestions) ? g9EmsPracticeQuestions : [];
    ...
    const FLAG_KEY = `g10_acct_st_flagged_${g9EmsPracticeSubskill || 'mixed'}`;
    ...
    const setAnswer = (idx, value) => {
        const answers = Array.isArray(g9EmsPracticeAnswers) ? [...g9EmsPracticeAnswers] : [];
        answers[idx] = value;
        setg9EmsPracticeAnswers(answers);

        const feedback = Array.isArray(g9EmsPracticeFeedback) ? [...g9EmsPracticeFeedback] : [];
        feedback[idx] = null;
        setg9EmsPracticeFeedback(feedback);
    };
    ...
    const renderOneJournalTable = ({ journal, q, idx, tableIndex = 0, showHighlights = false, memoEnabled = false, readOnlyMode = false, isReference = false }) => {
        ...
        const reviewResult = Array.isArray(g9EmsPracticeFeedback) ? g9EmsPracticeFeedback[idx] : null;
        ...
        const ans = (g9EmsPracticeAnswers?.[idx] && typeof g9EmsPracticeAnswers[idx] === 'object')
            ? g9EmsPracticeAnswers[idx]
    ...
    const renderQuestionContent = (question, idx, options = {}) => {
        ...
        const questionAnswer = g9EmsPracticeAnswers?.[idx];
    ...
    const feedbackByQuestion = Array.isArray(g9EmsPracticeFeedback) ? g9EmsPracticeFeedback : [];
    ...
    return (
        <div className="w-full">
            ...
            {isSuperAdmin && (
                <div className="mb-4 p-3 bg-amber-50 border border-amber-300 rounded-xl flex items-center gap-3 flex-wrap">
                    ...
                    <label className="flex items-center gap-2 text-sm text-amber-900 font-medium">
                        ...
                        <input
                            type="number"
                            value={g9EmsPracticeSeed || ''}
                            onChange={(e) => setg9EmsPracticeSeed(e.target.value)}
                    ...
                    <button
                        onClick={() => {
                            fetchGrade9EmsPractice({
                                subskill: g9EmsPracticeSubskill,
                                difficulty: g9EmsPracticeDifficulty,
                                seed: g9EmsPracticeSeed,
            ...
            {g9EmsPracticeError && (
                <div className="text-sm text-red-700 break-words">{g9EmsPracticeError}</div>
            )}

            {!g9EmsPracticeLoading && questions.length === 0 && (
...
export default Grade9EmsPractice;
