...
const Grade9FractionsPractice = ({
    onBack,
    g9FracVisualAidsOpen,
    setG9FracVisualAidsOpen,
    g9FracPracticeDifficulty,
    setG9FracPracticeDifficulty,
    fetchGrade9FractionsPractice,
    g9FracPracticeLoading,
    g9FracPracticeError,
    g9FracPracticeQuestions,
    g9FracPracticeAnswers,
    setG9FracPracticeAnswers,
    g9FracPracticeFeedback,
    setG9FracPracticeFeedback,
    renderGrade9FractionsVisualAids,
}) => {
    const questions = Array.isArray(g9FracPracticeQuestions) ? g9FracPracticeQuestions : [];

    const setAnswer = (idx, value) => {
        const answers = Array.isArray(g9FracPracticeAnswers) ? [...g9FracPracticeAnswers] : [];
        answers[idx] = value;
        setG9FracPracticeAnswers(answers);

        const feedback = Array.isArray(g9FracPracticeFeedback) ? [...g9FracPracticeFeedback] : [];
        feedback[idx] = null;
        setG9FracPracticeFeedback(feedback);
    };
    ...
    const checkOne = (idx) => {
        ...
        const expected = normalizeFractionAnswer(q.correct_answer);
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-white p-6 rounded-xl shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Grade 9 Mathematics • Fractions • Practice</h2>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-end md:justify-between mb-6">
                        <div className="w-full md:w-64">
                            ...
                            <select
                                value={g9FracPracticeDifficulty}
                                onChange={(e) => setG9FracPracticeDifficulty(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => fetchGrade9FractionsPractice({ difficulty: g9FracPracticeDifficulty })}
                                disabled={g9FracPracticeLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400"
                            >
                                {g9FracPracticeLoading ? 'Generating…' : 'Generate New Set'}
                            </button>
                        </div>
                    </div>

                    {g9FracPracticeError && (
                        <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">{g9FracPracticeError}</div>
                    )}

                    <div className="space-y-4">
                        {questions.map((q, idx) => {
                            const answerValue = (Array.isArray(g9FracPracticeAnswers) ? g9FracPracticeAnswers[idx] : '') ?? '';
                            const feedback = Array.isArray(g9FracPracticeFeedback) ? g9FracPracticeFeedback[idx] : null;
                        ...
                        {!g9FracPracticeLoading && questions.length === 0 && !g9FracPracticeError && (
                            <div className="p-6 text-gray-600">No questions yet.</div>
                        )}
                    </div>
                </div>

                <VisualAidsPanel isOpen={g9FracVisualAidsOpen} setIsOpen={setG9FracVisualAidsOpen}>
                    {renderGrade9FractionsVisualAids?.()}
                </VisualAidsPanel>
            </div>
        </div>
...
export default Grade9FractionsPractice;
