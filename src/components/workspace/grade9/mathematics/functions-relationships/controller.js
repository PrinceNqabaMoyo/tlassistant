...
export const useGrade9FunctionsRelationshipsController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const scaffoldSteps = useMemo(
        () => [
            { key: 'outputs_for_sets', title: 'Outputs from a rule', prompt: 'Apply a rule to a given set of inputs.' },
            { key: 'flow_table_outputs', title: 'Flow → outputs', prompt: 'Apply 1–2 operations to inputs to get outputs.' },
            { key: 'table_from_graph', title: 'Graph → table', prompt: 'Read y values from a straight-line graph.' },
            { key: 'symbolic_from_words', title: 'Words → formula', prompt: 'Write y = ... from a procedure.' },
            { key: 'word_from_symbolic', title: 'Formula → words', prompt: 'Explain y = ... in words.' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade9/functions-relationships/generate');
            ...
            if (!res.ok) throw new Error(`Grade 9 functions scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 9 functions scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade9/functions-relationships/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 9 Functions (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 9 Functions (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 9 Functions (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 9 Functions (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade9/functions-relationships/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_functions_relationships_1_scaffold') return;
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_functions_relationships_1_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade9_functions_relationships_1_scaffold' || workspaceMode === 'grade9_functions_relationships_1_practice';
        if (!inMode) return;
