...
export const useGrade9PatternsController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const scaffoldSteps = useMemo(
        () => [
            ...
            { key: 'square_numbers', title: 'Square numbers', prompt: 'Recognise and extend square-number sequences.' },
            { key: 'growing_difference', title: 'Growing differences', prompt: 'Look at the differences and how they change.' },
            { key: 'build_sequence_from_instruction', title: 'Build a sequence', prompt: 'Follow an instruction (add/subtract/multiply/divide) to make terms.' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade9/patterns/generate');
            ...
            if (!res.ok) throw new Error(`Grade 9 Patterns scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 9 patterns scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade9/patterns/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 9 Patterns (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 9 Patterns (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 9 Patterns (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 9 Patterns (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade9/patterns/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_patterns_scaffold') return;
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_patterns_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade9_patterns_scaffold' || workspaceMode === 'grade9_patterns_practice';
        if (!inMode) return;
