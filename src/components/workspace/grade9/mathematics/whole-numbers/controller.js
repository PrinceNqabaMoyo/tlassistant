...
export const useGrade9WholeNumbersController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const [visualAidsTab, setVisualAidsTab] = useState('number_systems');

    const scaffoldSteps = useMemo(
        () => [
            { key: 'number_systems_table', title: 'Number systems (closure)', prompt: 'Complete a closure table for different number sets.' },
            { key: 'rounding_estimation', title: 'Rounding & estimation', prompt: 'Estimate by rounding, then calculate.' },
            { key: 'long_division', title: 'Long division', prompt: 'Divide whole numbers and show the working.' },
            { key: 'prime_factorization', title: 'Prime factorisation', prompt: 'Write prime factors using exponent notation.' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade9/whole-numbers/generate');
            ...
            if (!res.ok) throw new Error(`Grade 9 whole numbers scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 9 whole numbers scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade9/whole-numbers/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 9 Whole Numbers (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 9 Whole Numbers (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 9 Whole Numbers (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 9 Whole Numbers (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade9/whole-numbers/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_whole_scaffold') return;
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_whole_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade9_whole_scaffold' || workspaceMode === 'grade9_whole_practice';
        if (!inMode) return;
        if (!visualAidsOpen) return;
        if (!visualAidsTab) setVisualAidsTab('number_systems');
