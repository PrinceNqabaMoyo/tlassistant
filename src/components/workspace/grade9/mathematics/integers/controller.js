...
export const useGrade9IntegersController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const scaffoldSteps = useMemo(
        () => [
            { key: 'properties_table', title: 'Properties (table)', prompt: 'Use the table to identify x or the property.' },
            { key: 'add_sub_integers', title: 'Add & subtract', prompt: 'Calculate with negative numbers (include subtraction of negatives).' },
            { key: 'multiply_divide_integers', title: 'Multiply & divide', prompt: 'Apply sign rules for × and ÷.' },
            { key: 'distributive_property', title: 'Distributive property', prompt: 'Use a(b + c) = ab + ac with negatives.' },
            { key: 'mixed_operations', title: 'Mixed operations', prompt: 'Use order of operations with brackets.' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade9/integers/generate');
            ...
            if (!res.ok) throw new Error(`Grade 9 integers scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 9 integers scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade9/integers/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 9 Integers (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 9 Integers (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 9 Integers (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 9 Integers (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade9/integers/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_integers_scaffold') return;
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_integers_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade9_integers_scaffold' || workspaceMode === 'grade9_integers_practice';
        if (!inMode) return;
        if (!visualAidsOpen) return;
        if (!visualAidsTab) setVisualAidsTab('rules');
