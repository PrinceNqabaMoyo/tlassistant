...
export const useGrade9ExponentsController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const [visualAidsTab, setVisualAidsTab] = useState('laws');

    const scaffoldSteps = useMemo(
        () => [
            { key: 'write_exponential_form', title: 'Exponential form', prompt: 'Write repeated multiplication as a power.' },
            { key: 'order_of_operations', title: 'Order of operations', prompt: 'Evaluate expressions with powers first.' },
            { key: 'laws_product_quotient', title: 'Laws (× and ÷)', prompt: 'Use laws for multiplying/dividing powers with same base.' },
            { key: 'power_of_a_power', title: 'Power of a power', prompt: 'Use (a^m)^n = a^(m×n).' },
            { key: 'zero_exponent', title: 'Zero exponent', prompt: 'Use a^0 = 1.' },
            { key: 'negative_exponent_meaning', title: 'Negative exponent meaning', prompt: 'Rewrite with positive exponents.' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade9/exponents/generate');
            ...
            if (!res.ok) throw new Error(`Grade 9 exponents scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 9 exponents scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade9/exponents/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 9 Exponents (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 9 Exponents (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 9 Exponents (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 9 Exponents (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade9/exponents/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_exponents_scaffold') return;
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade9_exponents_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade9_exponents_scaffold' || workspaceMode === 'grade9_exponents_practice';
        if (!inMode) return;
        if (!visualAidsOpen) return;
        if (!visualAidsTab) setVisualAidsTab('laws');
