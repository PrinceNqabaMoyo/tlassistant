...
const Grade10AccountingSoleTraderScaffold = ({
    onBack,
    scaffoldSteps,
    subskills,
    g10AcctSTVisualAidsOpen,
    setG10AcctSTVisualAidsOpen,
    g10AcctSTScaffoldDifficulty,
    setG10AcctSTScaffoldDifficulty,
    g10AcctSTSubskill,
    setG10AcctSTSubskill,
    g10AcctSTScaffoldSeed,
    setG10AcctSTScaffoldSeed,
    g10AcctSTScaffoldStepIndex,
    setG10AcctSTScaffoldStepIndex,
    fetchGrade10AcctSTScaffoldQuestion,
    g10AcctSTScaffoldLoading,
    g10AcctSTScaffoldError,
    g10AcctSTScaffoldQuestion,
    g10AcctSTScaffoldAnswer,
    setG10AcctSTScaffoldAnswer,
    g10AcctSTScaffoldFeedback,
    setG10AcctSTScaffoldFeedback,
    g10AcctSTScaffoldShowHint,
    setG10AcctSTScaffoldShowHint,
    ...
    const SR_KEY = `g10_acct_st_sr_queue_${(scaffoldSteps[g10AcctSTScaffoldStepIndex] || scaffoldSteps[0])?.key || 'mixed'}`;
    ...
    const question = g10AcctSTScaffoldQuestion;
    const selectedSubtopic = scaffoldSteps[g10AcctSTScaffoldStepIndex] || scaffoldSteps[0];
    ...
    const currentDifficulty = String(g10AcctSTScaffoldDifficulty || 'easy').trim().toLowerCase();
    ...
    const newExample = () => {
        ...
        if (shouldIncrementAnswerCount && nextCount % 5 === 0 && srQueue.length > 0) {
            ...
            setG10AcctSTScaffoldAnswer(null);
            setG10AcctSTScaffoldFeedback(null);
            setG10AcctSTScaffoldShowHint(false);
            // Use fetchGrade10AcctSTScaffoldQuestion trick: we override with a known question
            ...
            fetchGrade10AcctSTScaffoldQuestion({ _srOverride: replayQ });
            return;
        }
        ...
        fetchGrade10AcctSTScaffoldQuestion({
            ...
            seed: g10AcctSTScaffoldSeed,
        });
    };

    const setAnswerValue = (value) => {
        setG10AcctSTScaffoldAnswer(value);
        if (question) {
    ...
    const checkAnswer = () => {
        ...
        if (question.question_type === 'mcq') {
            const ok = String(g10AcctSTScaffoldAnswer) === String(question.correct_index);
            setShowCheckHighlights(true);
            setG10AcctSTScaffoldFeedback(ok
        ...
        if (question.question_type === 'typed') {
            const user = normalizeText(g10AcctSTScaffoldAnswer);
            if (!user) {
                setG10AcctSTScaffoldFeedback({ kind: 'error', message: 'Write an answer first.' });
                return;
            }
            setShowCheckHighlights(true);
            setG10AcctSTScaffoldFeedback({ kind: 'info', message: 'AI-graded feedback available in Pro subscription.' });
            return;
        }

        if (question.question_type === 'calc') {
            const userN = toNumber(g10AcctSTScaffoldAnswer);
            if (userN === null) {
                setG10AcctSTScaffoldFeedback({ kind: 'error', message: 'Enter a number first.' });
                return;
            }
        ...
        if (question.question_type === 'match') {
            ...
            const userMap = (g10AcctSTScaffoldAnswer && typeof g10AcctSTScaffoldAnswer === 'object') ? g10AcctSTScaffoldAnswer : {};
            let hit = 0;
            let total = 0;
            ...
            setShowCheckHighlights(true);
            setG10AcctSTScaffoldFeedback(hit === total 
                ? { kind: 'success', message: 'Correct.' }
        ...
        if (question.question_type === 'journal' || question.question_type === 'ledger') {
            ...
            const ans = (g10AcctSTScaffoldAnswer && typeof g10AcctSTScaffoldAnswer === 'object')
                ? g10AcctSTScaffoldAnswer
            ...
            if (!keys.length) {
                setG10AcctSTScaffoldFeedback({ kind: 'error', message: 'Nothing to mark for this journal question.' });
                return;
            }
            ...
            setShowCheckHighlights(true);
            setG10AcctSTScaffoldFeedback(isPerfect
                ? { kind: 'success', message: 'Correct.' }
        ...
        setG10AcctSTScaffoldFeedback({ kind: 'error', message: `Unsupported question type: ${question.question_type}` });
    };
    ...
    const checkAnswerWithFlagging = () => {
        ...
        setPreCheckAnswer(g10AcctSTScaffoldAnswer);
        checkAnswer();
        registerDifficultyAttempt();
        // After checking, inspect the feedback result to decide flagging
        // We use a micro-timeout so that setG10AcctSTScaffoldFeedback has settled
        setTimeout(() => {
            if (!question) return;
            ...
            const isCorrect = (() => {
                ...
                if (qt === 'mcq') return String(g10AcctSTScaffoldAnswer) === String(question.correct_index);
                if (qt === 'calc') {
                    const userN = toNumber(g10AcctSTScaffoldAnswer);
                    const correct = Number(question.correct_value);
                        ...
                        {rows.map((row, rIdx) => (
                            <tr key={rIdx}>
                                {(Array.isArray(row) ? row : []).map((cell, cIdx) => {
                                    ...
                                    const showCellHintButton = g10AcctSTScaffoldShowHint && marking.isPracticeMode && editable && cellHintSections.length > 0;
    ...
    return (
        <div className="w-full">
            ...
            {isSuperAdmin && (
                <div className="mb-4 p-3 bg-amber-50 border border-amber-300 rounded-xl flex items-center gap-3 flex-wrap">
                    ...
                    <label className="flex items-center gap-2 text-sm text-amber-900 font-medium">
                        ...
                        <input
                            type="number"
                            value={g10AcctSTScaffoldSeed || ''}
                            onChange={(e) => setG10AcctSTScaffoldSeed(e.target.value)}
                    ...
                    <button
                        onClick={() => {
                            ...
                            fetchGrade10AcctSTScaffoldQuestion({
                                subskill: effectiveSubskill,
                                difficulty: g10AcctSTScaffoldDifficulty,
                                seed: g10AcctSTScaffoldSeed,
            ...
            {!question && !g10AcctSTScaffoldLoading && (
            ...
            {question && (
                <div className="space-y-4" onKeyDown={handleAnswerKeyDown}>
                    ...
                    {g10AcctSTScaffoldShowHint && (question?.question_type === 'journal' || question?.question_type === 'ledger') && question?.journal?.column_help && Object.keys(question.journal.column_help).length > 0 && marking.isPracticeMode && (
                    ...
                    {g10AcctSTScaffoldShowHint && (question?.question_type === 'journal' || question?.question_type === 'ledger') && question?.cell_hints && Object.keys(question.cell_hints).length > 0 && marking.isPracticeMode && (
                    ...
                    {g10AcctSTScaffoldShowHint && question?.question_type === 'typed' && typedHintSections.length > 0 && marking.isPracticeMode && (
                    ...
                    {g10AcctSTScaffoldShowHint && question?.question_type === 'calc' && calcHintText && marking.isPracticeMode && (
                        <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
                    ...
                    {question.question_type === 'mcq' && (
                        <div className="grid grid-cols-1 gap-2">
                            {(Array.isArray(question.options) ? question.options : []).map((opt, idx) => {
                                const isSelected = String(g10AcctSTScaffoldAnswer) === String(idx);
                    ...
                    {question.question_type === 'calc' && (() => {
                        const gotN = toNumber(g10AcctSTScaffoldAnswer);
                        ...
                        return (
                            <div className="space-y-3">
                                <input
                                    ref={activeInputRef}
                                    value={showMemo ? question.correct_value : (g10AcctSTScaffoldAnswer || '')}
                                    onFocus={() => { if (localStorage.getItem('mathKeypadSettings_disableAutoPopup') !== 'true') setIsKeypadVisible(true); }}
                                    onChange={(e) => setAnswerValue(e.target.value)}
                    ...
                    {question?.question_type === 'match' && (
                        <MatchQuestionUI 
                            question={question}
                            answer={g10AcctSTScaffoldAnswer}
                    ...
                    <div className="mt-6 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                                ...
                                question && (
                                    ...
                                    onClick={checkAnswerWithFlagging}
                                        disabled={question?.question_type === 'typed'
                                            ? String(g10AcctSTScaffoldAnswer || '').trim() === ''
                                            : (!g10AcctSTScaffoldAnswer || String(g10AcctSTScaffoldAnswer).trim() === '')}
                            ...
                            <button
                                onClick={() => setG10AcctSTScaffoldShowHint(!g10AcctSTScaffoldShowHint)}
                                className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-800 rounded-lg text-sm font-medium transition-colors"
                            >
                                {g10AcctSTScaffoldShowHint ? 'Hide hint' : 'Hint'}
                            </button>
                            <button
                                onClick={() => setG10AcctSTScaffoldFeedback(null)}
                    ...
                    {g10AcctSTScaffoldFeedback && marking.isPracticeMode && question?.question_type !== 'typed' && (
                        <div className={`rounded-xl p-3 border text-sm ${g10AcctSTScaffoldFeedback.kind === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-900' : g10AcctSTScaffoldFeedback.kind === 'error' ? 'bg-red-50 border-red-200 text-red-900' : 'bg-slate-50 border-slate-200 text-slate-800'}`}>
                            <div className="font-semibold">Feedback</div>
                            <div className="text-sm mt-1">{g10AcctSTScaffoldFeedback.message}</div>
                        </div>
                    )}
                    {g10AcctSTScaffoldFeedback && marking.isPracticeMode && question?.question_type === 'typed' && (
...
export default Grade10AccountingSoleTraderScaffold;
