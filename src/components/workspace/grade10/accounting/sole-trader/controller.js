...
export const useGrade10SoleTraderController = ({ workspaceMode, buildApiUrl }) => {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_accounting_sole_trader_practice') return;
    ...
    useEffect(() => {
        const inMode = workspaceMode === 'grade10_accounting_sole_trader_scaffold' || workspaceMode === 'grade10_accounting_sole_trader_practice';
        if (!inMode) return;
