...
export const useGrade10BSMicroEnvironmentController = ({ workspaceMode, buildApiUrl }) => {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_bs_micro_environment_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade10_bs_micro_environment_scaffold' || workspaceMode === 'grade10_bs_micro_environment_practice';
        if (!inMode) return;
