...
export const useGrade10BSInterrelationshipController = ({ workspaceMode, buildApiUrl }) => {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_bs_interrelationship_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade10_bs_interrelationship_scaffold' || workspaceMode === 'grade10_bs_interrelationship_practice';
        if (!inMode) return;
