...
export const useGrade10BSBusinessFunctionsController = ({ workspaceMode, buildApiUrl }) => {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_bs_business_functions_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade10_bs_business_functions_scaffold' || workspaceMode === 'grade10_bs_business_functions_practice';
        if (!inMode) return;
