...
export const useGrade10BSEntrepreneurialQualitiesController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const scaffoldMode = 'grade10_bs_entrepreneurial_qualities_scaffold';
    const { steps: scaffoldSteps, loading: stepsLoading } = useDynamicScaffoldSteps({
        topicKey: 'grade10_bs_entrepreneurial_qualities',
        buildApiUrl,
        enabled: workspaceMode === scaffoldMode,
    });

    const endpointPath = '/api/business-studies/grade10/generate';
    const topicKey = 'grade10_bs_entrepreneurial_qualities';
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_bs_entrepreneurial_qualities_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade10_bs_entrepreneurial_qualities_scaffold' || workspaceMode === 'grade10_bs_entrepreneurial_qualities_practice';
        if (!inMode) return;
