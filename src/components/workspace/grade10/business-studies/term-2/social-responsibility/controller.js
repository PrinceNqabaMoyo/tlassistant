...
export const useGrade10BSSocialResponsibilityController = ({ workspaceMode, buildApiUrl }) => {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_bs_social_responsibility_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade10_bs_social_responsibility_scaffold' || workspaceMode === 'grade10_bs_social_responsibility_practice';
        if (!inMode) return;
