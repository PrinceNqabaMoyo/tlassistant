...
export const useGrade10BSConceptOfQualityController = ({ workspaceMode, buildApiUrl }) => {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_bs_concept_of_quality_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade10_bs_concept_of_quality_scaffold' || workspaceMode === 'grade10_bs_concept_of_quality_practice';
        if (!inMode) return;
