...
export const useGrade10BSFormsOfOwnershipController = ({ workspaceMode, buildApiUrl }) => {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_bs_forms_of_ownership_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade10_bs_forms_of_ownership_scaffold' || workspaceMode === 'grade10_bs_forms_of_ownership_practice';
        if (!inMode) return;
