...
export const useGrade10BSSocioEconomicIssuesController = ({ workspaceMode, buildApiUrl }) => {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_bs_socio_economic_issues_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade10_bs_socio_economic_issues_scaffold' || workspaceMode === 'grade10_bs_socio_economic_issues_practice';
        if (!inMode) return;
