...
const Grade10PatternsSequencesPractice = ({
    onBack,
    g10PatSeqVisualAidsOpen,
    setG10PatSeqVisualAidsOpen,
    g10PatSeqPracticeDifficulty,
    setG10PatSeqPracticeDifficulty,
    fetchGrade10PatternsSequencesPractice,
    g10PatSeqPracticeLoading, 
    g10PatSeqPracticeError,
    g10PatSeqPracticeQuestions,
    g10PatSeqPracticeAnswers,
    setG10PatSeqPracticeAnswers,
    g10PatSeqPracticeFeedback,
    setG10PatSeqPracticeFeedback,
    renderGrade10PatternsSequencesVisualAids,
}) => {
    const questions = Array.isArray(g10PatSeqPracticeQuestions) ? g10PatSeqPracticeQuestions : [];

    const checkOne = (idx) => {
        const q = questions[idx];
        if (!q) return;

        const answers = Array.isArray(g10PatSeqPracticeAnswers) ? [...g10PatSeqPracticeAnswers] : [];
        const feedback = Array.isArray(g10PatSeqPracticeFeedback) ? [...g10PatSeqPracticeFeedback] : [];
        ...
        setG10PatSeqPracticeFeedback(feedback);
    };

    const setAnswer = (idx, value) => {
        const answers = Array.isArray(g10PatSeqPracticeAnswers) ? [...g10PatSeqPracticeAnswers] : [];
        answers[idx] = value;
        setG10PatSeqPracticeAnswers(answers);
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-white p-6 rounded-xl shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Grade 10 Mathematics • Patterns &amp; Sequences • Practice</h2>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-end md:justify-between mb-6">
                        <div className="w-full md:w-64">
                            ...
                            <select
                                value={g10PatSeqPracticeDifficulty}
                                onChange={(e) => setG10PatSeqPracticeDifficulty(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => fetchGrade10PatternsSequencesPractice({ difficulty: g10PatSeqPracticeDifficulty })}
                                disabled={g10PatSeqPracticeLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400"
                            >
                                {g10PatSeqPracticeLoading ? 'Generating…' : 'Generate New Set'}
                            </button>
                        </div>
                    </div>
                    ...
                    {g10PatSeqPracticeLoading && questions.length === 0 && (
                        <div className="p-6 text-gray-600">Loading questions…</div>
                    )}

                    <div className="space-y-6">
                        {questions.map((q, idx) => {
                            const answerValue = (Array.isArray(g10PatSeqPracticeAnswers) ? g10PatSeqPracticeAnswers[idx] : '') ?? '';
                            const feedback = Array.isArray(g10PatSeqPracticeFeedback) ? g10PatSeqPracticeFeedback[idx] : null;
                        ...
                        {!g10PatSeqPracticeLoading && questions.length === 0 && !g10PatSeqPracticeError && (
...
export default Grade10PatternsSequencesPractice;
