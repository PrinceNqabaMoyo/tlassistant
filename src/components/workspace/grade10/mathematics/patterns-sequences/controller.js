...
export const useGrade10PatternsSequencesController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const scaffoldSteps = useMemo(
        () => [
            { key: 'next_terms_arithmetic', title: 'Arithmetic sequences', prompt: 'Find next terms using common difference d and T_n = a + (n − 1)d.' },
            { key: 'next_terms_geometric', title: 'Geometric sequences', prompt: 'Find next terms using a common ratio.' },
            { key: 'tables_seating', title: 'Tables seating pattern', prompt: 'Use a linear formula like T_n = 4 + 2(n − 1).' },
            { key: 'matchsticks_squares_in_row', title: 'Matchsticks squares', prompt: 'Use a linear rule like T_n = 3n + 1.' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade10/patterns-sequences/generate');
            ...
            if (!res.ok) throw new Error(`Grade 10 Patterns & Sequences scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 10 Patterns & Sequences scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade10/patterns-sequences/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 10 Patterns & Sequences (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 10 Patterns & Sequences (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 10 Patterns & Sequences (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 10 Patterns & Sequences (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade10/patterns-sequences/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade10_patterns_sequences_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inPatSeqMode = workspaceMode === 'grade10_patterns_sequences_scaffold' || workspaceMode === 'grade10_patterns_sequences_practice';
