...
const Grade10EquationsInequalitiesPractice = ({
    onBack,
    g10EqIneqVisualAidsOpen,
    setG10EqIneqVisualAidsOpen,
    g10EqIneqPracticeDifficulty,
    setG10EqIneqPracticeDifficulty,
    fetchGrade10EqIneqPractice,
    g10EqIneqPracticeLoading,
    g10EqIneqPracticeError,
    g10EqIneqPracticeQuestions,
    g10EqIneqPracticeAnswers,
    setG10EqIneqPracticeAnswers,
    g10EqIneqPracticeFeedback,
    setG10EqIneqPracticeFeedback,
    renderGrade10EqIneqVisualAids,
}) => {
    const questions = Array.isArray(g10EqIneqPracticeQuestions) ? g10EqIneqPracticeQuestions : [];

    const setAnswer = (idx, value) => {
        const answers = Array.isArray(g10EqIneqPracticeAnswers) ? [...g10EqIneqPracticeAnswers] : [];
        answers[idx] = value;
        setG10EqIneqPracticeAnswers(answers);

        const feedback = Array.isArray(g10EqIneqPracticeFeedback) ? [...g10EqIneqPracticeFeedback] : [];
        feedback[idx] = null;
        setG10EqIneqPracticeFeedback(feedback);
    };
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-white p-6 rounded-xl shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Grade 10 Mathematics • Equations &amp; inequalities • Practice</h2>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-end md:justify-between mb-6">
                        <div className="w-full md:w-64">
                            ...
                            <select
                                value={g10EqIneqPracticeDifficulty}
                                onChange={(e) => setG10EqIneqPracticeDifficulty(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => fetchGrade10EqIneqPractice({ difficulty: g10EqIneqPracticeDifficulty })}
                                disabled={g10EqIneqPracticeLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400"
                            >
                                {g10EqIneqPracticeLoading ? 'Generating…' : 'Generate New Set'}
                            </button>
                        </div>
                    </div>

                    {g10EqIneqPracticeError && (
                        <div className="mb-6 p-4 rounded-lg border border-red-200 bg-red-50 text-red-800 text-sm">{g10EqIneqPracticeError}</div>
                    )}

                    {g10EqIneqPracticeLoading && questions.length === 0 && (
                        <div className="p-6 text-gray-600">Loading questions…</div>
                    )}

                    <div className="space-y-6">
                        {questions.map((q, idx) => {
                            const userValue = (Array.isArray(g10EqIneqPracticeAnswers) ? g10EqIneqPracticeAnswers[idx] : '') ?? '';
                            const fb = Array.isArray(g10EqIneqPracticeFeedback) ? g10EqIneqPracticeFeedback[idx] : null;
                        ...
                        {!g10EqIneqPracticeLoading && questions.length === 0 && !g10EqIneqPracticeError && (
...
export default Grade10EquationsInequalitiesPractice;
