...
const Grade10ExponentsPractice = ({
    onBack,
    g10ExpVisualAidsOpen,
    setG10ExpVisualAidsOpen,
    g10ExpPracticeDifficulty,
    setG10ExpPracticeDifficulty,
    fetchGrade10ExpPractice,
    g10ExpPracticeLoading,
    g10ExpPracticeError,
    g10ExpPracticeQuestions,
    g10ExpPracticeAnswers,
    setG10ExpPracticeAnswers,
    g10ExpPracticeFeedback,
    setG10ExpPracticeFeedback,
    normalizeWholeNumberAnswer,
    normalizeTextAnswer,
    renderGrade10ExpVisualAids,
}) => {
    const questions = Array.isArray(g10ExpPracticeQuestions) ? g10ExpPracticeQuestions : [];
    ...
    const checkOne = (idx) => {
        ...
        const answers = Array.isArray(g10ExpPracticeAnswers) ? [...g10ExpPracticeAnswers] : [];
        const feedback = Array.isArray(g10ExpPracticeFeedback) ? [...g10ExpPracticeFeedback] : [];
        ...
        setG10ExpPracticeFeedback(feedback);
    };

    const setAnswer = (idx, value) => {
        const answers = Array.isArray(g10ExpPracticeAnswers) ? [...g10ExpPracticeAnswers] : [];
        answers[idx] = value;
        setG10ExpPracticeAnswers(answers);
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-white p-6 rounded-xl shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Grade 10 Mathematics • Exponents • Practice</h2>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-end md:justify-between mb-6">
                        <div className="w-full md:w-64">
                            ...
                            <select
                                value={g10ExpPracticeDifficulty}
                                onChange={(e) => setG10ExpPracticeDifficulty(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => fetchGrade10ExpPractice({ difficulty: g10ExpPracticeDifficulty })}
                                disabled={g10ExpPracticeLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400"
                            >
                                {g10ExpPracticeLoading ? 'Generating…' : 'Generate New Set'}
                            </button>
                        </div>
                    </div>

                    {g10ExpPracticeError && (
                        <div className="mb-6 p-4 rounded-lg border border-red-200 bg-red-50 text-red-800 text-sm">{g10ExpPracticeError}</div>
                    )}

                    {g10ExpPracticeLoading && questions.length === 0 && (
                        <div className="p-6 text-gray-600">Loading questions…</div>
                    )}

                    <div className="space-y-6">
                        {questions.map((q, idx) => {
                            const userValue = (Array.isArray(g10ExpPracticeAnswers) ? g10ExpPracticeAnswers[idx] : '') ?? '';
                            const fb = Array.isArray(g10ExpPracticeFeedback) ? g10ExpPracticeFeedback[idx] : null;
                                    ...
                                    {q.question_type === 'mcq' && Array.isArray(q.options) ? (
                                        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            {q.options.map((opt) => (
                                                <label
                                                    key={opt}
                                                    className="flex items-center gap-2 p-3 bg-white border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50"
                                                >
                                                    <input
                                                        type="radio"
                                                        name={`q_${idx}`}
                        ...
                        {!g10ExpPracticeLoading && questions.length === 0 && !g10ExpPracticeError && (
                            <div className="p-6 text-gray-600">No questions yet.</div>
                        )}
                    </div>
                </div>

                <VisualAidsPanel isOpen={g10ExpVisualAidsOpen} setIsOpen={setG10ExpVisualAidsOpen}>
                    {renderGrade10ExpVisualAids?.()}
                </VisualAidsPanel>
            </div>
        </div>
...
export default Grade10ExponentsPractice;
