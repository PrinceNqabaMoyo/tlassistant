...
export const useGrade11EquationsInequalitiesController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const scaffoldSteps = useMemo(
        () => [
            { key: 'completing_square', title: 'Completing the square' },
            { key: 'quadratic_formula', title: 'Quadratic formula' },
            { key: 'quadratic_inequalities', title: 'Quadratic inequalities' },
            { key: 'simultaneous_linear', title: 'Simultaneous equations (linear)' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade11/equations-inequalities/generate');
            ...
            if (!res.ok) throw new Error(`Grade 11 Equations & Inequalities scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 11 Equations & Inequalities scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade11/equations-inequalities/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 11 Equations & Inequalities (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 11 Equations & Inequalities (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 11 Equations & Inequalities (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 11 Equations & Inequalities (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade11/equations-inequalities/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade11_equations_inequalities_practice') return;
