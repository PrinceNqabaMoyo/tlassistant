...
export const useGrade11ExponentsSurdsController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const scaffoldSteps = useMemo(
        () => [
            { key: 'exponent_laws', title: 'Exponent laws' },
            { key: 'rational_exponent_to_radical', title: 'Rational exponents to radicals' },
            { key: 'simplify_surd', title: 'Simplify surds' },
            { key: 'rationalise_denominator', title: 'Rationalise denominators' },
            { key: 'exponential_application', title: 'Applications of exponentials' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade11/exponents-surds/generate');
            ...
            if (!res.ok) throw new Error(`Grade 11 Exponents & Surds scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 11 Exponents & Surds scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade11/exponents-surds/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 11 Exponents & Surds (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 11 Exponents & Surds (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 11 Exponents & Surds (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 11 Exponents & Surds (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade11/exponents-surds/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade11_exponents_surds_practice') return;
