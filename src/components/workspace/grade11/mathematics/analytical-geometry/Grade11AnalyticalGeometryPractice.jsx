...
const Grade11AnalyticalGeometryPractice = ({
    onBack,
    g11AnalGeoPracticeQuestions,
    g11AnalGeoPracticeAnswers,
    setG11AnalGeoPracticeAnswers,
    g11AnalGeoPracticeFeedback,
    setG11AnalGeoPracticeFeedback,
    g11AnalGeoPracticeLoading,
    g11AnalGeoPracticeError,
    g11AnalGeoPracticeDifficulty,
    setG11AnalGeoPracticeDifficulty,
    fetchGrade11AnalGeoPractice,
    g11AnalGeoVisualAidsOpen,
    setG11AnalGeoVisualAidsOpen,
    renderGrade11AnalGeoVisualAids,
}) => {
    const questions = Array.isArray(g11AnalGeoPracticeQuestions) ? g11AnalGeoPracticeQuestions : [];

    const setAnswerAt = (idx, value) => {
        setG11AnalGeoPracticeAnswers((prev) => {
            const next = Array.isArray(prev) ? [...prev] : [];
            next[idx] = value;
            return next;
        });
        setG11AnalGeoPracticeFeedback((prev) => {
            const next = Array.isArray(prev) ? [...prev] : [];
            next[idx] = null;
            return next;
        });
    ...
    const checkOne = (idx) => {
        ...
        if (!q) return;
        const answers = Array.isArray(g11AnalGeoPracticeAnswers) ? g11AnalGeoPracticeAnswers : [];
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-white p-6 rounded-xl shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Grade 11 Mathematics • Analytical Geometry • Practice</h2>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-end md:justify-between mb-6">
                        <div className="w-full md:w-64">
                            ...
                            <select
                                value={g11AnalGeoPracticeDifficulty}
                                onChange={(e) => setG11AnalGeoPracticeDifficulty(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => fetchGrade11AnalGeoPractice({ difficulty: g11AnalGeoPracticeDifficulty })}
                                disabled={g11AnalGeoPracticeLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400"
                            >
                                {g11AnalGeoPracticeLoading ? 'Loading…' : 'New Set'}
                            </button>
                        </div>
                    </div>

                    {g11AnalGeoPracticeError && (
                        <div className="mb-6 p-4 rounded-lg border border-red-200 bg-red-50 text-red-800 text-sm">{g11AnalGeoPracticeError}</div>
                    )}

                    <div className="space-y-6">
                        {questions.map((q, idx) => {
                            const answerValue = (Array.isArray(g11AnalGeoPracticeAnswers) ? g11AnalGeoPracticeAnswers[idx] : '') ?? '';
                            const feedback = Array.isArray(g11AnalGeoPracticeFeedback) ? g11AnalGeoPracticeFeedback[idx] : null;
                                                ...
                                                {q.options.map((opt) => (
                                                    <label key={opt} className="flex items-center gap-2 p-3 bg-white border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                                                        <input
                                                            ...
                                                            name={q.id || `q_${idx}`}
                                                            value={opt}
                                                            checked={String(answerValue) === String(opt)}
                        ...
                        {!g11AnalGeoPracticeLoading && questions.length === 0 && !g11AnalGeoPracticeError && (
...
export default Grade11AnalyticalGeometryPractice;
