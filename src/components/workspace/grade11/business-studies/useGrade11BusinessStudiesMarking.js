...
export const useGrade11BusinessStudiesMarking = (config = {}) => {
    const { buildApiUrl, apiUrl } = config;
    const resolvedApiUrl = useMemo(() => {
        if (typeof buildApiUrl === 'function') {
            return buildApiUrl('/api/business-studies/grade11/mark');
        }
        return apiUrl || buildApiUrl('/api/business-studies/grade11/mark');
    }, [buildApiUrl, apiUrl]);
    ...
    const markQuestions = useCallback(async (questions, answers) => {
        ...
        try {
            ...
            console.error('Failed to mark Grade 11 Business Studies questions:', err);
