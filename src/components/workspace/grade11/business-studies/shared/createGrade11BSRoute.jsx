...
import { useGrade11BusinessStudiesMarking } from '../useGrade11BusinessStudiesMarking';
...
import { createGrade11BSController } from './createGrade11BSController';
...
/**
 * Assembles the scaffold + practice registry entries for a single Grade 11
 * Business Studies topic from a small config object, keeping each new topic to a
 * few lines of declarative config. Mirrors the Grade 10 shared layer but uses
 * the Grade 11 marking hook (keyword-based mastery) and endpoints.
 */
export const createGrade11BSTopicRegistry = (config) => {
    const {
        topicKey,
        modePrefix,
        topicTitle,
    ...
    const useController = createGrade11BSController({ topicKey, modePrefix, scaffoldSteps });
