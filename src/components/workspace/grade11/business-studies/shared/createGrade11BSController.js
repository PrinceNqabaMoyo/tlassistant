...
/**
 * Factory that builds a controller hook for a Grade 11 Business Studies topic.
 ...
 * mirrors the Grade 10 shared layer but targets the Grade 11 endpoints.
 */
export const createGrade11BSController = ({
    topicKey,
    modePrefix,
    endpointPath = '/api/business-studies/grade11/generate',
    ...
    return ({ workspaceMode, buildApiUrl }) => {
        ...
        const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
            ...
            try {
                ...
                if (!res.ok) throw new Error(`Grade 11 Business Studies scaffold request failed: HTTP ${res.status}`);
