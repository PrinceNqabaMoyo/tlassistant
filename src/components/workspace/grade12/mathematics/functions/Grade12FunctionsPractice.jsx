...
const Grade12FunctionsPractice = ({
    onBack,
    g12FunctionsVisualAidsOpen,
    setG12FunctionsVisualAidsOpen,
    g12FunctionsPracticeDifficulty,
    setG12FunctionsPracticeDifficulty,
    fetchGrade12FunctionsPractice,
    g12FunctionsPracticeLoading,
    g12FunctionsPracticeError,
    g12FunctionsPracticeQuestions,
    g12FunctionsPracticeAnswers,
    setG12FunctionsPracticeAnswers,
    g12FunctionsPracticeFeedback,
    setG12FunctionsPracticeFeedback,
    renderGrade12FunctionsVisualAids,
}) => {
    const questions = Array.isArray(g12FunctionsPracticeQuestions) ? g12FunctionsPracticeQuestions : [];

    const checkOne = (idx) => {
        const q = questions[idx];
        if (!q) return;

        const answers = Array.isArray(g12FunctionsPracticeAnswers) ? [...g12FunctionsPracticeAnswers] : [];
        const feedback = Array.isArray(g12FunctionsPracticeFeedback) ? [...g12FunctionsPracticeFeedback] : [];
        ...
        setG12FunctionsPracticeFeedback(feedback);
    };

    const setAnswer = (idx, value) => {
        const answers = Array.isArray(g12FunctionsPracticeAnswers) ? [...g12FunctionsPracticeAnswers] : [];
        answers[idx] = value;
        setG12FunctionsPracticeAnswers(answers);
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-white p-6 rounded-xl shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Grade 12 Mathematics • Functions • Practice</h2>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-end md:justify-between mb-6">
                        <div className="w-full md:w-64">
                            ...
                            <select
                                value={g12FunctionsPracticeDifficulty}
                                onChange={(e) => setG12FunctionsPracticeDifficulty(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => fetchGrade12FunctionsPractice({ difficulty: g12FunctionsPracticeDifficulty })}
                                disabled={g12FunctionsPracticeLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400"
                            >
                                {g12FunctionsPracticeLoading ? 'Generating…' : 'Generate New Set'}
                            </button>
                        </div>
                    </div>
                    ...
                    {g12FunctionsPracticeLoading && questions.length === 0 && (
                        <div className="p-6 text-gray-600">Loading questions…</div>
                    )}

                    <div className="space-y-6">
                        {questions.map((q, idx) => {
                            const answerValue = (Array.isArray(g12FunctionsPracticeAnswers) ? g12FunctionsPracticeAnswers[idx] : '') ?? '';
                            const feedback = Array.isArray(g12FunctionsPracticeFeedback) ? g12FunctionsPracticeFeedback[idx] : null;
                        ...
                        {!g12FunctionsPracticeLoading && questions.length === 0 && !g12FunctionsPracticeError && (
...
export default Grade12FunctionsPractice;
