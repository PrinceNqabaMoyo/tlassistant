...
const Grade12TrigonometryPractice = ({
    onBack,
    g12TrigVisualAidsOpen,
    setG12TrigVisualAidsOpen,
    g12TrigPracticeDifficulty,
    setG12TrigPracticeDifficulty,
    fetchGrade12TrigPractice,
    g12TrigPracticeLoading,
    g12TrigPracticeError,
    g12TrigPracticeQuestions,
    g12TrigPracticeAnswers,
    setG12TrigPracticeAnswers,
    g12TrigPracticeFeedback,
    setG12TrigPracticeFeedback,
    renderGrade12TrigVisualAids,
}) => {
    const questions = Array.isArray(g12TrigPracticeQuestions) ? g12TrigPracticeQuestions : [];

    const checkOne = (idx) => {
        const q = questions[idx];
        if (!q) return;

        const answers = Array.isArray(g12TrigPracticeAnswers) ? [...g12TrigPracticeAnswers] : [];
        const feedback = Array.isArray(g12TrigPracticeFeedback) ? [...g12TrigPracticeFeedback] : [];
        ...
        setG12TrigPracticeFeedback(feedback);
    };

    const setAnswer = (idx, value) => {
        const answers = Array.isArray(g12TrigPracticeAnswers) ? [...g12TrigPracticeAnswers] : [];
        answers[idx] = value;
        setG12TrigPracticeAnswers(answers);
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-white p-6 rounded-xl shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Grade 12 Mathematics • Trigonometry • Practice</h2>
                            <p className="text-sm text-gray-600">Mixed reduction, identities, and compound angles.</p>
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-end md:justify-between mb-6">
                        <div className="w-full md:w-64">
                            ...
                            <select
                                value={g12TrigPracticeDifficulty}
                                onChange={(e) => setG12TrigPracticeDifficulty(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => fetchGrade12TrigPractice({ difficulty: g12TrigPracticeDifficulty })}
                                disabled={g12TrigPracticeLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400"
                            >
                                {g12TrigPracticeLoading ? 'Generating…' : 'Generate New Set'}
                            </button>
                        </div>
                    </div>
                    ...
                    {g12TrigPracticeLoading && questions.length === 0 && (
                        <div className="p-6 text-gray-600">Loading questions…</div>
                    )}

                    <div className="space-y-6">
                        {questions.map((q, idx) => {
                            const answerValue = (Array.isArray(g12TrigPracticeAnswers) ? g12TrigPracticeAnswers[idx] : '') ?? '';
                            const feedback = Array.isArray(g12TrigPracticeFeedback) ? g12TrigPracticeFeedback[idx] : null;
                        ...
                        {!g12TrigPracticeLoading && questions.length === 0 && !g12TrigPracticeError && (
...
export default Grade12TrigonometryPractice;
