...
const DEFAULT_SCAFFOLD_STEPS = [
    ...
    { key: 'application', title: 'Application (Scenario)' },
    { key: 'discussion', title: 'Semantic/Essay (Discussion)' },
];

/**
 * Factory that builds a controller hook for a Grade 12 Business Studies topic.
 * Each topic only differs by its backend topic key, workspace mode prefix and
 * (optionally) its scaffold steps, so the heavy lifting is shared here. This
 * mirrors the Grade 11 shared layer but targets the Grade 12 endpoints.
 */
export const createGrade12BSController = ({
    topicKey,
    modePrefix,
    endpointPath = '/api/business-studies/grade12/generate',
    ...
    return ({ workspaceMode, buildApiUrl }) => {
        ...
        const { steps, loading: stepsLoading } = useDynamicScaffoldSteps({
            ...
            sectionsEndpoint: '/api/business-studies/grade12/sections',
        });
        ...
        const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
            ...
            try {
                ...
                if (!res.ok) throw new Error(`Grade 12 Business Studies scaffold request failed: HTTP ${res.status}`);
        ...
        const fetchPractice = async ({ difficulty }) => {
            ...
            try {
                ...
                const applicationReq = fetch(endpoint, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ topic: topicKey, subskill: 'application', difficulty: difficulty || 'medium', count: 2 }),
                }).then(r => r.json());

                const discussionReq = fetch(endpoint, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ topic: topicKey, subskill: 'discussion', difficulty: difficulty || 'medium', count: 2 }),
                }).then(r => r.json());

                const [conceptsRes, applicationRes, discussionRes] = await Promise.all([conceptsReq, applicationReq, discussionReq]);
