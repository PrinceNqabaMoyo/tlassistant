...
const Grade7StraightLinesPractice = ({
    onBack,
    g7StraightVisualAidsOpen,
    setG7StraightVisualAidsOpen,
    g7StraightPracticeDifficulty,
    setG7StraightPracticeDifficulty,
    fetchGrade7StraightLinesPractice,
    g7StraightPracticeLoading,
    g7StraightPracticeError,
    g7StraightPracticeQuestions,
    g7StraightPracticeAnswers,
    setG7StraightPracticeAnswers,
    g7StraightPracticeFeedback,
    setG7StraightPracticeFeedback,
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-white p-6 rounded-xl shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Grade 7 Mathematics • Geometry of straight lines • Practice</h2>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-end md:justify-between mb-6">
                        <div className="w-full md:w-64">
                            ...
                            <select
                                value={g7StraightPracticeDifficulty}
                                onChange={(e) => setG7StraightPracticeDifficulty(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => fetchGrade7StraightLinesPractice({ difficulty: g7StraightPracticeDifficulty })}
                                disabled={g7StraightPracticeLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400"
                            >
                                {g7StraightPracticeLoading ? 'Generating…' : 'Generate New Set'}
                            </button>
                        </div>
                    </div>
                    ...
                    {g7StraightPracticeLoading && g7StraightPracticeQuestions.length === 0 && (
                        <div className="p-6 text-gray-600">Loading questions…</div>
                    )}

                    <div className="space-y-6">
                        {g7StraightPracticeQuestions.map((q, idx) => {
                            const qid = q.id || q.question_id || `q_${idx}`;
                            const answerValue = g7StraightPracticeAnswers[qid] ?? '';
                            const feedback = g7StraightPracticeFeedback[qid];
                                            ...
                                            {q.options.map((opt) => (
                                                <label key={opt} className="flex items-center gap-2 p-3 bg-white border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-100">
                                                    <input
                                                        ...
                                                        onChange={(e) => setG7StraightPracticeAnswers((prev) => ({ ...prev, [qid]: e.target.value }))}
                                                    />
                                    ...
                                    <div className="mt-4 flex flex-col sm:flex-row gap-2 sm:items-center">
                                        <button
                                            onClick={() => {
                                                ...
                                                setG7StraightPracticeFeedback((prev) => ({
                                                    ...prev,
                                                    [qid]: {
                                                        isCorrect: ok,
                        ...
                        {!g7StraightPracticeLoading && g7StraightPracticeQuestions.length === 0 && !g7StraightPracticeError && (
...
export default Grade7StraightLinesPractice;
