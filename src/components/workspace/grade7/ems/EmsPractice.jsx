...
const EmsPractice = ({
    ...
    fetchPractice,
    practiceLoading, practiceError, practiceQuestions,
    practiceAnswers, setPracticeAnswers,
    practiceFeedback, setPracticeFeedback,
    renderVisualAids, hideConfig,
    ...
    const setAnswer = (idx, value) => {
        const answers = Array.isArray(practiceAnswers) ? [...practiceAnswers] : [];
        answers[idx] = value;
        setPracticeAnswers(answers);
        if (questions[idx]) marking.registerAnswer(questions[idx].id, value);
        const fb = Array.isArray(practiceFeedback) ? [...practiceFeedback] : [];
        fb[idx] = null;
    ...
    useEffect(() => {
        ...
        if (!Array.isArray(practiceAnswers) || practiceAnswers.length !== questions.length)
            setPracticeAnswers(questions.map((question) => {
        ...
        if (!Array.isArray(practiceFeedback) || practiceFeedback.length !== questions.length)
            setPracticeFeedback(questions.map(() => null));
    }, [questions.length]);

    const setFeedbackAt = (idx, value) => {
        const next = Array.isArray(practiceFeedback) ? practiceFeedback.slice() : [];
        next[idx] = value;
        setPracticeFeedback(next);
    };
    ...
    const checkOne = (q, idx) => {
        if (!q) return;
        const ans = Array.isArray(practiceAnswers) ? practiceAnswers[idx] : null;
    ...
    const renderOneStructuredTable = (q, idx, table, tableIndex, { readOnly = false, cellValues = null } = {}) => {
        ...
        const rows = Array.isArray(table?.rows) ? table.rows : [];
        const ans = (practiceAnswers?.[idx] && typeof practiceAnswers[idx] === 'object') ? practiceAnswers[idx] : buildEmptyTableAnswer(q);
    ...
    const renderWordbankTable = (q, idx, { readOnly = false, useCorrectAnswers = false } = {}) => {
        const answerState = (practiceAnswers?.[idx] && typeof practiceAnswers[idx] === 'object')
            ? practiceAnswers[idx]
    ...
    const renderReviewQuestion = (reviewQuestion, idx) => {
        const reviewFeedback = Array.isArray(practiceFeedback) ? practiceFeedback[idx] : null;
        const reviewAnswer = Array.isArray(practiceAnswers) ? practiceAnswers[idx] : null;
    ...
    const ans = practiceAnswers?.[currentIndex];
    const feedback = practiceFeedback?.[currentIndex];
    ...
    return (
        <>
            ...
            {practiceLoading && <div className="text-sm text-slate-500">Loading...</div>}
            {practiceError && <div className="text-sm text-red-700 break-words">{practiceError}</div>}
            {!practiceLoading && questions.length === 0 && <div className="text-sm text-slate-500">Click "Generate Question" to start.</div>}
