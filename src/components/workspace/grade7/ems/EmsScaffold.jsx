...
const EmsScaffold = ({
    ...
    visualAidsOpen, setVisualAidsOpen,
    ...
    scaffoldAnswer, setScaffoldAnswer,
    scaffoldFeedback, setScaffoldFeedback,
    scaffoldShowHint, setScaffoldShowHint,
    renderVisualAids, hideConfig, evaluationState,
    isMarkingEnv,
}) => {
    const question = scaffoldQuestion;
    ...
    const selectedSubtopic = scaffoldSteps[scaffoldStepIndex] || scaffoldSteps[0];
    ...
    }, [scaffoldStepIndex, isMarkingEnv]);
    ...
    useEffect(() => {
        if (!question) return;
        if (isFillInTableQuestionType(question?.question_type) && (!scaffoldAnswer || typeof scaffoldAnswer !== 'object' || !scaffoldAnswer.cells)) {
            setScaffoldAnswer(buildEmptyTableAnswer(question));
        }
        if (question.question_type === 'table_wordbank' && (!scaffoldAnswer || typeof scaffoldAnswer !== 'object' || !scaffoldAnswer.selections)) {
            setScaffoldAnswer(buildEmptyWordbankAnswer(question));
        }
    }, [question?.id]);

    const newExample = () => {
        fetchScaffoldQuestion({ subskill: selectedSubtopic?.key || 'closing_transfers', difficulty: scaffoldDifficulty });
        setShowMemo(false);
    };
    ...
    const checkAnswer = () => {
        ...
        if (question.question_type === 'mcq') {
            const ok = String(scaffoldAnswer) === String(question.correct_index);
            setScaffoldFeedback(ok ? { kind: 'success', message: 'Correct.' } : { kind: 'error', message: `Not quite. Correct: ${question.options?.[question.correct_index] || ''}` });
            return;
        }
        if (question.question_type === 'calc') {
            const expected = parseFloat(question.correct_answer);
            const got = parseFloat(scaffoldAnswer);
            if (isNaN(got)) { setScaffoldFeedback({ kind: 'error', message: 'Enter a number.' }); return; }
            const ok = Math.abs(expected - got) < 0.01;
            setScaffoldFeedback(ok ? { kind: 'success', message: 'Correct!' } : { kind: 'error', message: `Not quite. Expected: R${expected.toFixed(2)}` });
            return;
        }
        if (question.question_type === 'table_wordbank') {
            const correctMap = getCorrectMap(question);
            const ans = (scaffoldAnswer && typeof scaffoldAnswer === 'object') ? scaffoldAnswer : buildEmptyWordbankAnswer(question);
            const selections = ans?.selections && typeof ans.selections === 'object' ? ans.selections : {};
            let total = 0;
            let hit = 0;
            ...
            setScaffoldFeedback(ok ? { kind: 'success', message: 'Correct.' } : { kind: 'error', message: `Not quite. You matched ${hit}/${total} correctly.` });
            return;
        }
        if (question.question_type === 'typed') {
            if (!normalizeText(scaffoldAnswer)) { setScaffoldFeedback({ kind: 'error', message: 'Write an answer, then check.' }); return; }
            setScaffoldFeedback({ kind: 'info', message: 'Answer submitted. Use Compare / Memo to review the memo and guidance.' });
            return;
        }
        if (isFillInTableQuestionType(question.question_type)) {
            const correctMap = getCorrectMap(question);
            const ans = (scaffoldAnswer && typeof scaffoldAnswer === 'object') ? scaffoldAnswer : buildEmptyTableAnswer(question);
            ...
            setScaffoldFeedback(ok ? { kind: 'success', message: 'Correct.' } : { kind: 'error', message: `Not quite. You completed ${hit}/${keys.length} cells correctly.` });
            return;
        }
        setScaffoldFeedback({ kind: 'info', message: 'Answer saved.' });
    };

    const setAnswerValue = (value) => {
        setScaffoldAnswer(value);
                        ...
                        {rows.map((row, rowIndex) => (
                            <tr key={rowIndex}>
                                {(Array.isArray(row) ? row : []).map((cell, cellIndex) => {
                                    ...
                                    const showCellHintButton = scaffoldShowHint && marking.isPracticeMode && editable && cellHintSections.length > 0;
                    ...
                    <div className="lg:col-span-2">
                        <div className="flex flex-col sm:flex-row sm:items-end gap-3">
                            <div className="flex-1">
                                ...
                                <select value={scaffoldStepIndex} onChange={(e) => { setScaffoldStepIndex(Number(e.target.value) || 0); setScaffoldFeedback(null); setScaffoldShowHint(false); setScaffoldAnswer(null); }} className="mt-1 w-full p-2 border rounded-lg">
                                    {scaffoldSteps.map((s, i) => <option key={s.key} value={i}>{s.title}</option>)}
                                </select>
                            </div>
                            <div>
                                <label className="text-sm font-semibold text-slate-700">Difficulty</label>
                                <select value={scaffoldDifficulty} onChange={(e) => setScaffoldDifficulty(e.target.value)} className="mt-1 p-2 border rounded-lg">
                                    <option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option>
                                </select>
                            </div>
                            <button onClick={newExample} className="px-4 py-2 bg-slate-900 text-white rounded-lg font-semibold hover:bg-slate-800" disabled={scaffoldLoading}>{scaffoldLoading ? 'Loading…' : 'New Example'}</button>
                        </div>
                    </div>
                </div>
            )}

            {scaffoldError && <div className="mb-3 p-3 bg-red-50 border border-red-200 text-red-800 rounded-lg text-sm break-words">{scaffoldError}</div>}
            {scaffoldLoading && <div className="text-sm text-slate-500">Loading...</div>}
                ...
                <div className="space-y-4">
                    {question.question_type === 'mcq' && (
                        <div className="mt-3 grid grid-cols-1 gap-2">
                            {(question.options || []).map((opt, idx) => (
                                <MCQOption key={idx} selected={String(scaffoldAnswer) === String(idx)} onClick={() => setAnswerValue(String(idx))} label={opt} />
                            ))}
                        </div>
                    )}
                    ...
                    {question.question_type === 'calc' && (
                        <div className="mt-3 flex items-center gap-2">
                            ...
                            <input type="number" step="0.01" className="w-full max-w-xs p-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-400" placeholder="0.00" value={scaffoldAnswer || ''} onChange={(e) => setAnswerValue(e.target.value)} />
                        </div>
                    )}

                    {question.question_type === 'typed' && (
                        <div className="mt-3">
                            <textarea value={scaffoldAnswer || ''} onChange={(e) => setAnswerValue(e.target.value)} placeholder="Write your answer..." className="w-full min-h-[120px] p-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-slate-300" />
                        </div>
                    )}
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                        {marking.isPracticeMode ? (
                            <>
                                ...
                                <button type="button" onClick={() => setScaffoldShowHint(!scaffoldShowHint)} className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-800 rounded-xl text-sm font-semibold transition-all">Hint</button>
                                {scaffoldFeedback && (['typed', 'calc', 'table_wordbank'].includes(String(question?.question_type || '')) || isFillInTableQuestionType(question?.question_type)) && (
                                ...
                                {(scaffoldFeedback || showMemo || scaffoldShowHint) && (
                                    <button type="button" onClick={() => { setScaffoldFeedback(null); setScaffoldShowHint(false); setShowMemo(false); setShowCheckHighlights(false); setActiveCellHint(null); }} className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-800 rounded-xl text-sm font-semibold transition-all">
                                        Clear feedback
                                    </button>
                                )}
                            </>
                    ...
                    {scaffoldShowHint && question?.question_type === 'typed' && typedHintSections.length > 0 && marking.isPracticeMode && (
                    ...
                    {scaffoldShowHint && question?.question_type === 'calc' && calcHintText && marking.isPracticeMode && (
                    ...
                    {scaffoldShowHint && question?.question_type === 'table_wordbank' && Array.isArray(question?.guidelines) && question.guidelines.length > 0 && marking.isPracticeMode && (
                        <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
                    ...
                    {scaffoldShowHint && isFillInTableQuestionType(question?.question_type) && Array.isArray(question?.guidelines) && question.guidelines.length > 0 && marking.isPracticeMode && (
                        <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
                    ...
                    {scaffoldShowHint && question?.question_type !== 'typed' && question?.question_type !== 'calc' && question?.question_type !== 'table_wordbank' && !isFillInTableQuestionType(question?.question_type) && marking.isPracticeMode && <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 text-yellow-900 rounded-xl text-sm">Use the visual aids panel for formulas, closing-transfer flow, and adjustment logic.</div>}

                    {scaffoldFeedback && marking.isPracticeMode && (
                        <div className={`mt-3 p-3 rounded-xl text-sm border ${scaffoldFeedback.kind === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-900' : scaffoldFeedback.kind === 'error' ? 'bg-red-50 border-red-200 text-red-900' : 'bg-slate-50 border-slate-200 text-slate-800'}`}>
                            {scaffoldFeedback.message}
                        </div>
                    )}
