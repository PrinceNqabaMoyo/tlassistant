...
export const useGrade8EmsController = ({ workspaceMode, buildApiUrl, config }) => {
    ...
    const endpointPath = config?.endpoint || '/api/grade8_ems/generate';
    ...
    const { steps: scaffoldSteps, loading: stepsLoading } = useDynamicScaffoldSteps({
        ...
        sectionsEndpoint: '/api/grade8_ems/sections',
    });
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            ...
            if (!res.ok) throw new Error(`Grade 8 EMS scaffold request failed: HTTP ${res.status}`);
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!res.ok) throw new Error(`Grade 8 EMS practice request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Generation failed');
