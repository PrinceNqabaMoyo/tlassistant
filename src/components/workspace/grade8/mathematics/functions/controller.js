...
export const useGrade8FunctionsController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const scaffoldSteps = useMemo(
        () => [
            { key: 'constant_or_variable', title: 'Constant vs variable', prompt: 'Identify whether quantities are constant or variable.' },
            { key: 'flow_table_outputs', title: 'Flow diagrams', prompt: 'Apply operator(s) to inputs to get outputs.' },
            { key: 'symbolic_from_words', title: 'Words → symbols', prompt: 'Write a symbolic formula y = ... from a word description.' },
            { key: 'word_from_symbolic', title: 'Symbols → words', prompt: 'Explain a symbolic formula in words.' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade8/functions/generate');
            const scaffoldCapable = new Set(['flow_table_outputs']);
            ...
            if (!res.ok) throw new Error(`Grade 8 Functions scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 8 functions scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade8/functions/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 8 Functions (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 8 Functions (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 8 Functions (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 8 Functions (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade8/functions/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        const inMode = workspaceMode === 'grade8_functions_1_scaffold' || workspaceMode === 'grade8_functions_1_practice';
        if (!inMode) return;
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade8_functions_1_practice') return;
