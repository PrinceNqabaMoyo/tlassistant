...
export const useGrade8IntegersController = ({ workspaceMode, buildApiUrl }) => {
    ...
    const [visualAidsTab, setVisualAidsTab] = useState('number_line');

    const scaffoldSteps = useMemo(
        () => [
            { key: 'number_line_fill', title: 'Number line', prompt: 'Fill missing integers on a number line / sequence.' },
            { key: 'compare_integers', title: 'Compare integers', prompt: 'Use < or > to compare integers.' },
            { key: 'add_sub_integers', title: 'Add & subtract', prompt: 'Add and subtract integers using sign rules.' },
            { key: 'multiply_divide_integers', title: 'Multiply & divide', prompt: 'Multiply/divide integers using sign rules.' },
        ],
        []
    );
    ...
    const fetchScaffoldQuestion = async ({ subskill, difficulty }) => {
        ...
        try {
            const endpoint = buildApiUrl('/api/math/grade8/integers/generate');

            const scaffoldCapable = new Set(['add_sub_integers']);
            ...
            if (!res.ok) throw new Error(`Grade 8 integers scaffold request failed: HTTP ${res.status}`);
            const data = await res.json();
            if (!data?.success) throw new Error(data?.error || 'Grade 8 integers scaffold generation failed');
            ...
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setScaffoldError(`Failed to fetch (${buildApiUrl('/api/math/grade8/integers/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    const fetchPractice = async ({ difficulty }) => {
        ...
        try {
            ...
            if (!typedRes.ok) throw new Error(`Grade 8 Integers (typed) request failed: HTTP ${typedRes.status}`);
            const typedData = await typedRes.json();
            if (!typedData?.success) throw new Error(typedData?.error || 'Grade 8 Integers (typed) generation failed');
            ...
            if (!mcqRes.ok) throw new Error(`Grade 8 Integers (mcq) request failed: HTTP ${mcqRes.status}`);
            const mcqData = await mcqRes.json();
            if (!mcqData?.success) throw new Error(mcqData?.error || 'Grade 8 Integers (mcq) generation failed');
            ...
            const msg = err?.message || String(err);
            if (String(msg).toLowerCase().includes('failed to fetch')) {
                setPracticeError(`Failed to fetch (${buildApiUrl('/api/math/grade8/integers/generate')}). Check that the backend is running and VITE_API_BASE_URL / runtime-config.js points to it.`);
            } else {
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade8_integers_scaffold') return;
    ...
    useEffect(() => {
        if (workspaceMode !== 'grade8_integers_practice') return;
        fetchPractice({ difficulty: practiceDifficulty });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [workspaceMode]);

    useEffect(() => {
        const inMode = workspaceMode === 'grade8_integers_scaffold' || workspaceMode === 'grade8_integers_practice';
        if (!inMode) return;
