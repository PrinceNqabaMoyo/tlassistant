...
const Grade8IntegersPractice = ({
    onBack,
    g8IntVisualAidsOpen,
    setG8IntVisualAidsOpen,
    g8IntPracticeDifficulty,
    setG8IntPracticeDifficulty,
    fetchGrade8IntegersPractice,
    g8IntPracticeLoading,
    g8IntPracticeError,
    g8IntPracticeQuestions,
    g8IntPracticeAnswers,
    setG8IntPracticeAnswers,
    g8IntPracticeFeedback,
    setG8IntPracticeFeedback,
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-white p-6 rounded-xl shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Grade 8 Mathematics • Integers • Practice</h2>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-end md:justify-between mb-6">
                        <div className="w-full md:w-64">
                            ...
                            <select
                                value={g8IntPracticeDifficulty}
                                onChange={(e) => setG8IntPracticeDifficulty(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => fetchGrade8IntegersPractice({ difficulty: g8IntPracticeDifficulty })}
                                disabled={g8IntPracticeLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400"
                            >
                                {g8IntPracticeLoading ? 'Generating…' : 'Generate New Set'}
                            </button>
                        </div>
                    </div>
                    ...
                    {g8IntPracticeLoading && g8IntPracticeQuestions.length === 0 && (
                        <div className="p-6 text-gray-600">Loading questions…</div>
                    )}

                    <div className="space-y-6">
                        {g8IntPracticeQuestions.map((q, idx) => {
                            const qid = q.id || q.question_id || `q_${idx}`;
                            const answerValue = g8IntPracticeAnswers[qid] ?? '';
                            const feedback = g8IntPracticeFeedback[qid];
                                                ...
                                                {q.options.map((opt) => (
                                                    <label key={opt} className="flex items-center gap-2 p-3 bg-white border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                                                        <input
                                                            ...
                                                            onChange={(e) => {
                                                                setG8IntPracticeAnswers((prev) => ({ ...prev, [qid]: e.target.value }));
                                                            }}
                                                        />
                        ...
                        {!g8IntPracticeLoading && g8IntPracticeQuestions.length === 0 && !g8IntPracticeError && (
...
export default Grade8IntegersPractice;
